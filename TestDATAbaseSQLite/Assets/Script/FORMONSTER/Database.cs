...
public partial class Database : MonoBehaviour
{
    ...
    class accounts
    {
        ...
        public string HistoryFungusAcaunt { get; set; }   // c помощью { get; set; } тут происходит магия и появляется в SQL данная Ячейка
    }
    ...
    class monsters    /// orcan
    {
        ...
        [Indexed] //// добавить индекс в учетную запись, чтобы избежать полного сканирования при загрузке символов 
        public string nameEdit { get; set; } // Можно переименовать 
        public string myNameEdit { get; set; } // Можно переименовать 
        public string description { get; set; } // Можно переименовать
        public string level { get; set; }
        public string status { get; set; }

        public string discriptionHide { get; set; }
        public string recomendation { get; set; }
        public string element { get; set; }

    }

    class itemsBattle    /// orcan
    {
        //[PrimaryKey] // // важно для производительности: O (log n) вместо O (n)
        //[AutoIncrement]
    ...
    //     public int ID { get; set; }
     ...
     //   [PrimaryKey] // // важно для производительности: O (log n) вместо O (n)
     //   [Collation("NOCASE")] //// [COLLATE NOCASE для сравнения без учета регистра. таким образом, мы не можем одновременно создавать "Арчер" и "Арчер" как символы]
        public string nameMonster { get; set; }  // имя в названия в Ячейки тлько 1 может быть
                                                 // [Indexed] //// добавить индекс в учетную запись, чтобы избежать полного сканирования при загрузке символов 
        public string levelMonster { get; set; } // Можно переименовать 
        public string locationColor { get; set; } // Можно переименовать 
        public string result { get; set; } // Можно переименовать
    ...
    class characters
    {
        ...
        public string HistoryFungusCharacter { get; set; }   // orcan c помощью { get; set; } тут происходит магия и появляется в SQL данная Ячейка
    }
    class character_inventory
    {
        ...
        public string HistoryFungusInvent { get; set; }   // c помощью { get; set; } тут происходит магия и появляется в SQL данная Ячейка
    }
    ...
    public bool TryLogin(string account, string password)
    {
        ...
        if (!string.IsNullOrWhiteSpace(account) && !string.IsNullOrWhiteSpace(password))
        {
            ...
            if (connection.FindWithQuery<accounts>("SELECT * FROM accounts WHERE name=? AND password=? and banned=0", account, password) != null)
            {
                ...
                string FungusHistory = connection.ExecuteScalar<string>("SELECT HistoryFungusAcaunt FROM accounts WHERE name=?", account); //orcan вытаскиваем переменную из аккаунта 
    ...
    // => all the other values can be read with CharacterLoad!     //  вот тут вытаскиваем из аакаунта Героев его
    ...
    //public GameObject CharacterLoad(string characterName, List<Player> prefabs, bool isPreview) // вот тут при выборе Героя Происходит СОздания на карте героя ПОтом  заполнения его характеристиками Разными
    //{
    ...
    //        //    player.HistoryFungusAcaunt      = row.HistoryFungusAcaunt;  // orcan
    //            player.HistoryFungusCharacter   = row.HistoryFungusCharacter;  // orcan
    //         //   player.HistoryFungusInvent      = row.HistoryFungusInvent;  // orcan
    ...
    //    connection.Execute("DELETE FROM character_inventory WHERE character=?", player.name); // сначало все удаляет у Игрока 
    ...
    //       HistoryFungusCharacter =  player.HistoryFungusCharacter  // orcan
    ...
    //        CharacterSave(player, online, false); // вот тут самое то что сохраняет все настройки пресонажа 
    ...
    //public string nameMonsterHide { get; set; }
    //public string monsterDescription { get; set; }
    //public string monsterDescriptionPlus { get; set; }
    //public string elemenMon { get; set; }
    //public string recommendationItem { get; set; }
    ...
    //public void PullValuesMonsters(string valueName, ItemManager managerSQLitem)
    //{
    //    Debug.Log("!!tebleName!! " + valueName);



    //    foreach (monsters row in connection.Query<monsters>("SELECT * FROM monsters  WHERE nameMonster=?", valueName))
    //    {


    //        managerSQLitem.monstersD.Add("nameMonsterHide", row.nameMonsterHide);

    //        string value = "";
    //        if (managerSQLitem.monstersD.TryGetValue("nameMonsterHide", out value))
    //        {

    //            Debug.Log("!!!managerSQLitem.monstersD" + value);
    ...
    /// <param name="itemManagerGO"></param>
